...
public class AuthenticationService implements IAuthenticationService {
    ...
    public User login(User user) throws UserNotFoundException, NotCorrectPasswordException {
        ...
        if (foundUser == null) {
            throw new UserNotFoundException("Пользователь с таким логином (" + user.getLogin() + ") не найден");
        }
        if (!user.getPassword().equals(foundUser.getPassword())) {
            throw new NotCorrectPasswordException("Пароль пользователя (" + user.getLogin() + ") введен неверно!");
        }
